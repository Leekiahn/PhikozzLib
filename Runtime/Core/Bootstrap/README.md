...
`BootstrapConfig`에 등록된 서비스들은 씬이 시작되기 전에 자동으로 생성·등록되며  
...
`IServiceInit`을 구현한 서비스에 한해서만 `InitAsync()`을 호출하고, 모든 초기화가 끝나면 `Bootstrapper.WaitUntilReadyAsync()`를 완료합니다.  
다른 서비스를 참조하거나 비동기 로딩이 필요한 초기화 로직은 `Awake()`가 아니라 `IServiceInit.InitAsync()`에 작성하세요.

...
<img width="598" height="1073" alt="Image" src="https://github.com/user-attachments/assets/afcb64ef-df88-4fdc-979c-e8c7331dec2f" />

> `AudioManager`는 PhikozzLib에서 **BGM**, **SFX/UI/Other**, **트랙 제어**, **페이드 처리**를 담당하는 오디오 서비스입니다.  
...
- 전체 사운드 일괄 제어
- 트랙 페이드 인/아웃
- 오디오 데이터베이스(`PlaylistDatabase`, `AudioDatabase`)를 인스펙터에서 직접 참조합니다.
- FEEL에셋의 `MMSoundManager`와 `MMSMPlaylistManager`를 통해 오디오 소스를 풀링합니다.  
프리팹 하위에 `MMSoundManager`와 `MMSMPlaylistManager`를 배치해야 합니다.
...
| `Play(string groupName, string soundName, Vector3 position = default, Transform attachToTransform = null)` | 이름으로 사운드를 찾아 위치 기반 또는 Transform 부착 방식으로 재생합니다. |
